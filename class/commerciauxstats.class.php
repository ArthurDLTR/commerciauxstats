...
/**
 *      Class to manage statisticsa for invoice 
 */
class CommerciauxStats extends Stats
{
    public $socid;
    public $userid;
     ...
     */
    public function __construct(DoliDB $db, $socid, $userid = 0, $typentid = 0, $categid = 0)
    {
        global $user, $conf;
        ...
        $this->userid = $userid;
        $this->join = ' LEFT JOIN dolibarr.llx_element_element AS e ON e.fk_target = f.rowid LEFT JOIN dolibarr.llx_commande AS c ON c.rowid = e.fk_source ';
        ...
        $this->where .= " AND e.targettype = 'facture' AND e.sourcetype = 'commande'";
        if(!$userid){
            $this->where .= " AND c.fk_user_author = ".$userid;
        }
    ...
    public function getNbByMonth($year, $format = 0)
    {
        ...
        $sql = "SELECT date_format(f.datec,'%m') as dm, COUNT(*) as nb";
        $sql.= " FROM ".$this->from;
        $sql.= $this->join;
	...
	public function getAverageByMonth($year)
	{
		...
		$sql = "SELECT date_format(datef,'%m') as dm, AVG(c.".$this->field.")";
		$sql .= " FROM ".$this->from;
		$sql .= $this->join;
		$sql .= " WHERE f.datef BETWEEN '".$this->db->idate(dol_get_first_day($year))."' AND '".$this->db->idate(dol_get_last_day($year))."'";
		$sql .= " AND ".$this->where;
